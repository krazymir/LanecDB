...
describe('utils', function () {
    it('Should return a whole number, larger than or equal to min and less than max', function () {
        let value = utils.getRandomRange(1000, 100000)
        assert(!isNaN(value) && value >= 1000 && value < 100000 && Math.floor(value) === value)
    })
    it('Should return null if the ip is not valid', function () {
        assert.equal(utils.encodeNick('not an ip', 1232))
    })
    it('Should return null if the ip is not valid', function () {
        assert.equal(utils.encodeNick('299.999.333.111', 1232))
    })
    it('Should return null if the port is not a number', function () {
        assert.equal(utils.encodeNick('192.168.0.1', 'not a port'))
    })
    it('Should return null if the port is less than 1000', function () {
    ...
    it('Should return the same ip/port', function () {
        let ip = '87.34.55.231'
        let port = 1234
